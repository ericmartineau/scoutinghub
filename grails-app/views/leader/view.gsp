        ...
        <s:section class="floatSection myprofile">
            <s:sectionHeader icon="profile-icon" code="leader.profile.myprofile">
                <s:ctxmenu>
                    ...
                    <g:ctxmenuItem>
                        <s:linker img="add-icon" title="${message(code:'leader.profile.addScoutingId')}" lbwidth="500" class="lightbox"
    ...
    <s:section class="floatSection">
        <s:sectionHeader icon="units-icon" code="leader.profile.groups">
            <s:ctxmenu>
                ...
                <g:ctxmenuItem>
                    <s:linker img="add-icon" title="${message(code: 'leaderGroup.permissions', args:[leader])}" controller="leaderGroup" action="permissions" id="${leader.id}" class="lightbox">
                        <g:inlineIcon class="add-icon"/>
                        <g:ctxmenuLabel>
            ...
            <g:if test="${leader?.groups?.size()}">
                <g:each in="${leader.groups}" var="group" status="i">
                    ...
                    <s:leaderUnit leaderGroup="${group}" code="${group?.leaderPosition}.label" class="${currClass}">
                        ...
                        <p:canAdministerGroup leader="${leader}" scoutGroup="${group?.scoutGroup}">(admin)</p:canAdministerGroup>
