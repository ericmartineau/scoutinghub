...
<g:form action="saveProfile">
    ...
    <s:section span="6" class="floatSection myprofile">
        <s:sectionHeader icon="profile-icon" code="${leader.toString()}">
            <s:ctxmenu>
        ...
        <s:propertyList class="form-horizontal edit-profile">
            ...
            <g:else>
                ...
                <s:property text="true" code="leader.address.label">${leader?.address1 ?: "Not set"}</s:property>
...
<s:section span="6" class="floatSection">
    ...
    <s:propertyList class="row-fluid">
        <g:if test="${leader?.groups?.size()}">
            <g:each in="${leader.groups}" var="group" status="i">
                ...
                <s:leaderUnit leaderGroup="${group}" code="${group?.leaderPosition}.label" class="${currClass} span6">
                    ${group?.scoutGroup}
