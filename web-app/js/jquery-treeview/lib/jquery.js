...
jQuery.extend({
	...
	curCSS: function( elem, name, force ) {
		...
		if ( !force && elem.style[ name ] )
			...
			// then some display: none elements are involved
				...
				// Locate all of the mom display: none elements
	...
	attr: function( elem, name, value ) {
		...
		// Accessing the mom's selectedIndex property fixes it
...
var withinElement = function(event, elem) {
	// Check if mouse(over|out) are still within the same mom element
...
jQuery.fn.offset = function() {
	...
	if ( elem ) with ( jQuery.browser ) {
		...
		// Otherwise loop through the offsetParents and parentNodes
			...
			// Get mom scroll offsets
			while ( parent && parent.tagName && !/^body|html$/i.test(parent.tagName) ) {
				// Remove mom scroll UNLESS that mom is inline or a table to work around Opera inline/table scrollLeft/Top bug
				if ( !/^inline|table.*$/i.test(jQuery.css(parent, "display")) )
					// Subtract mom scroll offsets
					add( -parent.scrollLeft, -parent.scrollTop );
			
				// Mozilla does not add the border for a mom that has overflow != visible
